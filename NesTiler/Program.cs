...
public class Program
{
    ...
    public static int Main(string[] args)
    {
        try
        {
            ...
            // CSV output
            List<string>? outTilesCsvLines = !string.IsNullOrEmpty(c.OutTilesCsv) ? new List<string>() : null;
            List<string>? outPalettesCsvLines = !string.IsNullOrEmpty(c.OutPalettesCsv) ? new List<string>() : null;
            ...
            for (var i = 0; i < c.FixedPalettes.Length; i++)
            {
                if (c.FixedPalettes[i] == null)
                    continue;
                SKColor[] colorsInPalette = c.FixedPalettes[i]!.ToArray();
                for (var j = 0; j < colorsInPalette.Length; j++)
                    colorsInPalette[j] = nesColors.FindSimilarColor(colorsInPalette[j]);
                c.FixedPalettes[i] = new Palette(colorsInPalette, nosort: true);
            }

            // Loading images
            foreach (KeyValuePair<int, string> imageFile in c.ImageFiles)
            {
                ...
                Match match = offsetRegex.Match(imageFile.Value);
                ...
                if (image == null)
                    throw new InvalidDataException($"Can't load {filename}.");
                images[imageFile.Key] = image;

                if (image.Width % c.TileWidth != 0)
                    throw new ArgumentException($"Image width must be divisible by {c.TileWidth}.", filename);
                if (image.Height % c.TileHeight != 0)
                    throw new ArgumentException($"Image height must be divisible by {c.TileHeight}.", filename);
            }
            ...
            foreach (var imageNum in images.Keys)
            {
                ...
                FastBitmap image = images[imageNum];
                for (var y = 0; y < image.Height; y++)
                {
                    for (var x = 0; x < image.Width; x++)
                    {
                        ...
                        if (color.Alpha >= 0x80)
                        {
                            SKColor similarColor = nesColors.FindSimilarColor(color);
                        ...
                        } else
                        {
                            if (!c.BgColor.HasValue)
                                throw new InvalidDataException("You must specify background color for images with transparency.");
            ...
            } else
            {
                ...
                {
                    FastBitmap image = images[imageNum];
                    for (var tileY = 0; tileY < image.Height / c.TilePalHeight; tileY++)
                    {
                        for (var tileX = 0; tileX < image.Width / c.TilePalWidth; tileX++)
                        {
                            // Count each color only once per tile/sprite
                            var colorsInTile = new List<SKColor>();
                            for (var y = 0; y < c.TilePalHeight; y++)
                            {
                                for (var x = 0; x < c.TilePalWidth; x++)
                                {
                                    SKColor color = image.GetPixelColor(tileX * c.TilePalWidth + x, tileY * c.TilePalHeight + y);
                                    if (!colorsInTile.Contains(color))
                                        colorsInTile.Add(color);
                                }
                            }

                            foreach (SKColor color in colorsInTile)
                            {
                ...
                // Most used colors
                SKColor[] candidates = colorPerTileCounter.OrderByDescending(kv => kv.Value).Select(kv => kv.Key).ToArray();
                ...
                for (var i = 0; i < Math.Min(candidates.Length, MAX_BG_COLOR_AUTODETECT_ITERATIONS); i++)
                {
                ...
                IOrderedEnumerable<KeyValuePair<SKColor, (Palette[] Palettes, Palette.LossyInfo? LossyInfo)>> kvAllLossless = calcResults.Where(kv => kv.Value.LossyInfo == null).OrderBy(kv => kv.Value.Palettes.Length);
                if (kvAllLossless.Any())
                {
                    // Lossless combinations found, get best
                    KeyValuePair<SKColor, (Palette[] Palettes, Palette.LossyInfo? LossyInfo)> kv = kvAllLossless.First();
                    (bgColor, calculatedPalettes) = (kv.Key, kv.Value.Palettes);
                } else
                {
                    // Lossy combinations found
                    KeyValuePair<SKColor, (Palette[] Palettes, Palette.LossyInfo? LossyInfo)> kvLossy = calcResults.OrderBy(kv => kv.Value.Palettes.Length).First();
            ...
            if (calculatedPalettes.Length > maxCalculatedPaletteCount)
            {
                ...
                if (c.LossyLevel <= 2)
                    throw new InvalidDataException($"Can't fit {calculatedPalettes.Length} palettes, {maxCalculatedPaletteCount} is maximum.");
                // Just warning
            ...
            foreach (var imageNum in images.Keys)
            {
                ...
                FastBitmap image = images[imageNum];
                _ = c.AttributeTableYOffsets.TryGetValue(imageNum, out var attributeTableOffset);
                ...
                for (var tilePalY = 0; tilePalY < (int)Math.Ceiling((image.Height + attributeTableOffset) / (float)c.TilePalHeight); tilePalY++)
                {
                    for (var tilePalX = 0; tilePalX < image.Width / c.TilePalWidth; tilePalX++)
                    {
                        ...
                        {
                            if (palettes[paletteIndex] == null)
                                continue;
                            var delta = palettes[paletteIndex]!.GetTileDelta(
                        ...
                        for (var y = 0; y < c.TilePalHeight; y++)
                        {
                            for (var x = 0; x < c.TilePalWidth; x++)
                            {
                                ...
                                if (cy < 0)
                                    continue;
                                SKColor color = image.GetPixelColor(tilePalX * c.TilePalWidth + x, cy, bgColor);
                                SKColor similarColor = bestPalette != null
                                    ? nesColors.FindSimilarColor(Enumerable.Concat(
                                            bestPalette,
                                            new[] { bgColor }
                                        ), color)
                                    : bgColor;
            ...
            foreach (var imageNum in c.OutAttributeTable.Keys)
            {
                ...
                var width = paletteIndexes[imageNum].GetLength(0);
                var height = paletteIndexes[imageNum].GetLength(1);
                for (var ptileY = 0; ptileY < Math.Ceiling(height / 2.0); ptileY++)
                {
                    for (var ptileX = 0; ptileX < Math.Ceiling(width / 2.0); ptileX++)
                    {
                        byte topLeft = 0;
                        byte topRight = 0;
                        byte bottomLeft = 0;
            ...
            foreach (var imageNum in images.Keys)
            {
                ...
                FastBitmap image = images[imageNum];
                _ = c.AttributeTableYOffsets.TryGetValue(imageNum, out var attributeTableOffset);
                if (!patternTables.ContainsKey(!c.SharePatternTable ? imageNum : 0))
                    patternTables[!c.SharePatternTable ? imageNum : 0] = new();
                Dictionary<Tile, int> patternTable = patternTables[!c.SharePatternTable ? imageNum : 0];
                if (!patternTablesReversed.ContainsKey(!c.SharePatternTable ? imageNum : 0))
                    patternTablesReversed[!c.SharePatternTable ? imageNum : 0] = new();
                Dictionary<int, Tile> patternTableReversed = patternTablesReversed[!c.SharePatternTable ? imageNum : 0];
                if (!nameTables.ContainsKey(imageNum))
                    nameTables[imageNum] = new List<int>();
                List<int> nameTable = nameTables[imageNum];
                ...
                for (var tileY = 0; tileY < image.Height / c.TileHeight; tileY++)
                {
                    for (var tileX = 0; tileX < image.Width / c.TileWidth; tileX++)
                    {
                        var tileData = new byte[c.TileWidth * c.TileHeight];
                        byte paletteID = 0;
                        for (var y = 0; y < c.TileHeight; y++)
                            for (var x = 0; x < c.TileWidth; x++)
                            {
                                SKColor color = image.GetPixelColor(tileX * c.TileWidth + x, tileY * c.TileHeight + y);
                                ...
                                {
                                    colorIndex = 1;
                                    while (palette![colorIndex] != color)
                                        colorIndex++;
                                }
                        ...
                        if (noGroup)
                        {
                            ...
                            tileID++;
                        } else if (patternTable.TryGetValue(tile, out var id))
                        {
                            if (c.Mode == Config.TilesMode.Backgrounds)
                                nameTable.Add(id);
                            currentTileID = id;
                ...
                if (tileID > 256)
                    throw new InvalidDataException("Tiles out of range.");
                ...
                if (c.OutPatternTable.ContainsKey(imageNum) && !c.SharePatternTable)
                {
                    ...
                    for (var t = c.PatternTableStartOffsets[imageNum]; t < tileID; t++)
                    {
            ...
            {
                Dictionary<int, Tile> patternTableReversed = !c.DoNotGroupTiles.Any()
                    ? patternTables[0].ToDictionary(kv => kv.Value, kv => kv.Key)
                    : patternTablesReversed[0];
                var patternTableRaw = new List<byte>();
                for (var t = c.PatternTableStartOffsetShared; t < tileID; t++)
                {
                    var raw = patternTableReversed[t].GetAsPatternData();
                    patternTableRaw.AddRange(raw);
                }
        ...
        catch (Exception ex) when (ex is InvalidDataException or InvalidOperationException or ArgumentOutOfRangeException or FileNotFoundException)
        {
            Console.Error.WriteLine($"Error. {ex.Message}");
            return 1;
        }
    ...
    private static (Palette[] palettes, Palette.LossyInfo? lossyInfo) CalculatePalettes(Dictionary<int, FastBitmap> images, bool[] paletteEnabled, Palette?[] fixedPalettes, Dictionary<int, int> attributeTableOffsets, int tilePalWidth, int tilePalHeight, SKColor bgColor)
    {
        IEnumerable<bool> required = Enumerable.Range(0, 4).Select(i => paletteEnabled[i] && fixedPalettes[i] == null);
        ...
        {
            FastBitmap image = images[imageNum];
            _ = attributeTableOffsets.TryGetValue(imageNum, out var attributeTableOffset);
            // For each tile/sprite
            for (var tileY = 0; tileY < (image.Height + attributeTableOffset) / tilePalHeight; tileY++)
            {
                for (var tileX = 0; tileX < image.Width / tilePalWidth; tileX++)
                {
        ...
        while (true)
        {
            ...
            foreach (Palette palette2 in resultPalettes)
                foreach (Palette palette1 in resultPalettes)
                {
            ...
            var grouped = false;
            foreach (Palette? t in top)
            {
                if (paletteCounter[t] > 0 && t.Count < 3)
                {
                    foreach (Palette p in resultPalettes)
                    {
                        IEnumerable<SKColor> newColors = p.Where(c => !t.Contains(c));
            ...
            if (!grouped)
                break; // Nothing changed, stop iterations
        }
